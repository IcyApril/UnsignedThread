...
$id = htmlspecialchars(mysql_real_escape_string($_GET['id'])); //get id if directly linked to.
if ($_POST['id']) { $id = htmlspecialchars(mysql_real_escape_string($_POST['id'])); }
$authkey = htmlspecialchars(mysql_real_escape_string($_POST['authkey']));

if (!$id) {
	$id = $_SESSION['id'];
} else {
if ($id != $_SESSION['id']) { unset($_SESSION['id']); unset($_SESSION['authkey']); }
}

$hashedauthkey = $_SESSION['auth'];

$replypost = htmlspecialchars(mysql_real_escape_string($_POST['replypost']));

if (($id) &&($hashedauthkey || $authkey)) {
if (!$hashedauthkey) {
$hashedauthkey =  hash('sha512', $authkey.$auth_salt);
}
$sql= 'SELECT * FROM `posts` WHERE id='.$id.' AND auth="'.$hashedauthkey.'"';
...
// HANDLE REPLIES
if ($replypost) {
if ($status == 3) { die('You cannot reply to a closed post. :(');}

$hashedauthkey =  hash('sha512', $authkey.$auth_salt);
$sql= 'SELECT * FROM `posts` WHERE id='.$_SESSION['id'].' AND auth="'.$_SESSION['auth'].'"';
...
$sql = 'INSERT INTO replies (pid, role, dt, post) VALUES ('.$id.', "guest", NOW(), "'.$replypost.'")';
if (!mysql_query($sql))  {
  $error = TRUE;
  } else {$replied = TRUE;
  mysql_query('UPDATE posts SET status=2 WHERE id='.$id);
  }
...
<?php include('includes/nav.php'); ?>
    ...
    <div class="container">
      ...
      <div class="hero-unit">
        <h1>Chat with <?=$sitename?> - View thread.</h1>
      </div>
...
<?php } ?>
<p><a class="btn" href="killsession.php">Log-out of thread &raquo;</a></p>
...
<h3><a rel="tooltip" title="Enter your Auth key, you created this to access your thread." id="authkeytooltip">Auth Key</a>:</h3>
